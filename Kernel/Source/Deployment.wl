    ...
    Enclose[
      path =
        If[manifest === Automatic,
          Confirm[
            First[
              FileNames[
                "webapps-manifest." ~~ ("m" | "wl" | "json" | "yaml" | "yml"),
                "/deployment"
              ],
              $Failed
            ],
      ...
      ConfirmMatch[
        Replace[
          path,
          {
            p_String?(StringEndsQ[#, ".yml" | ".yaml"]&) :> Import[p, "YAML"][
              "webapps"
            ],
            p_String?(StringEndsQ[#, ".json"]&) :> Import[p, "RawJSON"][
              "webapps"
            ],
            p_String?(StringEndsQ[#, ".m" | ".wl"]&) :> Import[p, "WL"],
            Except[_String] :> (
              Return[
                Failure[
                  "InvalidManifest",
      ...
      Map[
        Function[
          ...
          ResourceFunction["WithMessageHandler"][
            ...
            printMsg[StringJoin[ #["Information"], " | ", #["Tag"]]]&
          ]
        ],
        repos
      ];
    ...
    Enclose[
      ...
      If[OptionValue["DeployFrontend"],
        ...
        Which[
          ...
          True,
            Confirm @ DeployWebappFrontEnd[feLoc, repositoryAssoc["prefix"]]
        ],
        WWE`Logger[
          "WARN",
          "WWE",
      ...
      If[OptionValue["DeployBackend"],
        ...
        If[StringQ[deployWL],
          Confirm @ DeployWebappBackend[deployWL, "Initialize" -> init],
          WWE`Logger[
            "WARN",
            "WWE",
            "DeployWebappRepository",
...
DeployExpression[expr_, location_String : Automatic, OptionsPattern[]] :=
  Module[{
    ...
    Enclose[
      dir =
        FileNameJoin[
          {OptionValue["WebappLocation"], OptionValue["ActiveExtension"], loc}
        ];
...
DeployRouteMap[apiMap_Association, acc_String] :=
  KeyValueMap[Function[{k, v}, DeployRouteMap[v, StringJoin[ acc, k]]], apiMap];
DeployRouteMap[expr : Except[_Association], acc_String] :=
  DeployExpression[expr, acc];
...
DeployWebappFrontEnd[feLoc_, location_String : "", OptionsPattern[]] :=
  Enclose[
    Block[{
        ...
        buildCommand =
          StringRiffle[
            {StringJoin[ "cd ", feLoc], "bun install", "bun build:wwe"},
            "&&"
          ],
        deployLoc = FileNameJoin[{OptionValue["WebappLocation"], location}]
      },
      Print["\n - Deploying Frontend - \n"];
      Pause[0.01];
      (* Run build command *)
      ...
      ConfirmAssert[
        Run[StringTemplate["cp -r `1`/* `2`"][buildLoc, deployLoc]] === 0,
    ...
    Enclose[
      ...
      Switch[repositoryAssoc["type"],
        ...
        "url:paclet",
          ...
          pacletClone[repositoryAssoc["name"], repositoryAssoc["remote"]],
        "site:archive",
          log[
            StringJoin[
          ...
          archiveSiteClone[repositoryAssoc["remote"], repositoryAssoc["local"]],
        "url:archive",
          log[
            StringJoin[
              "Cloning zip archive from '",
          ...
          archiveClone[repositoryAssoc["remote"], repositoryAssoc["local"]],
        "file",
          log[StringJoin[ "Files exists at '", repositoryAssoc["local"], "'"]];
          repositoryAssoc["local"],
        _,
          $Failed
      ]
        ...
        SortBy[
          ...
          Function[
            With[{
              ...
              -Total[ResourceFunction["VersionOrder"][v, #]& /@ versions]
            ]
          ]
        ];
      ConfirmMatch[
...
siteClone[name_String, site_String] :=
  Enclose[
    Quiet @ PacletUninstall[name];
    Confirm[
      PacletInstall[name, PacletSite -> site, ForceVersionInstall -> True],
...
ContinuousDeploymentWebhookHandler[] :=
  Module[{
      ...
      httpBodyIn,
      httpHeadersIn,
      log = WWE`Logger["INFO", "WWE", "ContinuousDeploymentWebhookHandler", #]&
    },
    Enclose[
      ...
      ConfirmMatch[
        httpBodyIn =
          Association @ ImportString[HTTPRequestData["Body"], "RawJSON"],
        _Association,
        StringJoin[ "Body is not an association: ", ToString[httpBodyIn]]
      ];
      ConfirmMatch[
        httpHeadersIn =
          Association @ ImportString[HTTPRequestData["Headers"], "RawJSON"],
        _Association,
        StringJoin[ "Headers are not an association: ", ToString[httpHeadersIn]]
      ];
      (* Act on request *)
    ...
    Enclose[
      ...
      If[event === "pr:merged",
        ...
        ConfirmBy[
          link =
            SelectFirst[
              httpBody["pullRequest", "toRef", "repository", "links", "clone"],
              #name === "ssh"&
            ][
              "href"
            ],
          StringQ,
          StringJoin[ "Clone link ", ToString[link], " is not a string"]
        ];
        ...
        If[
          And[
            repoAssoc =!= <||>,
            mergeDest === (StringJoin[ "refs/heads/", repoAssoc["branch"]])
          ],
